...
pub struct ConnConfig {
    ...
    /// Порог молчания — двух разных видов (V5, 2026-09-17; доработка
    /// 2026-09-24). (1) Ни одного кадра вовсе, даже `pong`, дольше
    /// `recv_timeout` — транспорт действительно мёртв (TCP мог остаться
    /// живым, а данных, включая протокольные, не будет вовсе), соединение
    /// рвётся и переподключается. (2) Кадры (`pong`) идут, а `Book`/`Trade`
    /// нет дольше `recv_timeout` — решение владельца 24.09: это нормальная
    /// тишина неликвидной монеты, не повод рвать соединение; только
    /// предупреждение в `stderr` и `ConnEvent::MarketSilence` для счётчика
...
pub enum ConnEvent {
    ...
    /// Молчание рынка (`Book`/`Trade`) дольше `recv_timeout` при живом
    /// транспорте (V5, доработка 2026-09-24 — решение владельца 24.09:
    /// тишина по неликвидной монете нормальна, соединение по ней не рвётся).
    /// Не разрыв: транспорт жив, подписка не трогается, книга не сбрасывается
    /// — поэтому не `Disconnected` и в `gaps.csv` строки не даёт (`feed::live`
    /// считает его отдельно, тем же приёмом, что `Unrouted`). Один на эпизод
    /// молчания — от первого превышения порога до следующего `Book`/`Trade`,
    /// который его кончает. `silence_ns` — тишина на момент обнаружения (сама
    /// проверка идёт раз в `ping_interval`, см. `run_with_backoff`): пока
    /// эпизод не кончился, его итоговая длина не измерена, поэтому это нижняя
    /// оценка, не окончательный итог.
    MarketSilence { local_ts_ns: i64, silence_ns: i64 },
}
...
impl<C: TransportConnector> Connection<C> {
    ...
    async fn run_with_backoff(
        ...
        loop {
            ...
            // `recv_timeout`.
            //
            // Решение владельца 24.09: тишина по неликвидной монете —
            // нормальное явление, не повод рвать соединение (переподключение
            // не возвращает рынку сделки, только теряет время на рукопожатие
            // и рискует поймать разрыв аккурат в момент, когда кадр наконец
            // пришёл бы). Раньше эта ветка звала `broadcast_disconnected` и
            // обрывала цикл (`break`); теперь порог даёт только наблюдение —
            // предупреждение в stderr не чаще одного на эпизод молчания
            // (`silence_warned` ниже) и `ConnEvent::MarketSilence` для
            // счётчика эпизодов и максимума в `session.json`. Мёртвый
            // транспорт (закрытый фрейм, ошибка сокета, неудавшийся пинг) —
            // по-прежнему разрыв: это не тишина рынка, а смерть сокета.
            ...
            // Один эпизод молчания — одно предупреждение (решение владельца
            // выше): взводится при первом превышении порога в ветке пинга,
            // снимается первым же `Book`/`Trade` — там же, где двигается
            // `last_market_ns`.
            ...
            loop {
                tokio::select! {
                    ...
                    _ = ping_due.tick() => {
                        ...
                        if silence_ns > recv_timeout_ns && !silence_warned {
                            // Решение владельца 24.09: не рвать — только
                            // предупредить и посчитать (см. комментарий у
                            // `recv_timeout` выше). `silence_warned` держит
                            // это одноразовым на эпизод; снимается первым же
                            // `Book`/`Trade`.
                            let names: Vec<&str> =
