...
//!
//! Оговорка про `u` (важно): суточный файл шага 0.3 `u` не хранит (`order_id`/`ival`
//! заняты другим, см. `record.rs`), поэтому файловый реплей идёт с синтетическими
//! порядковыми `u` и покрывает проверки 2-3. Проверка 1 требует настоящего `u` и
