...
/// Максимум уровней на сторону, который мы держим. Быстрый поток
/// (`orderbook.50`, `bybit::conn::ORDERBOOK_DEPTH`) даёт пятьдесят; глубокий
/// (`orderbook.200`, `bybit::conn::ORDERBOOK_DEEP_DEPTH`) — двести. Запас нужен
/// потому, что дельта может назвать цену, вышедшую за топ, и мы обязаны её
/// удалить, а не проигнорировать.
///
/// 256, не 128: двумстам уровням одной стороны ёмкости 128 не хватает —
/// снапшот `.200` дорастал бы её удвоениями посреди горячего пути, а живой A/B
/// (`docs/findings/depth-200-ab-2026-09-15.md`, «Как мерилось») собрал `.200`
/// именно с `128 → 256`. Двести — длина снапшота топика (документация Bybit
/// v5), 256 — ближайшая степень двойки сверху: та же политика запаса, что была
/// у 128 при пятидесяти уровнях (2.56 × против 2.5 ×), и она же даёт место
/// дельте за пределами топа.
...
pub struct Update {
    ...
    /// Глубина потока, из топика которого пришло обновление: у
    /// `orderbook.50.<symbol>` — 50, у `orderbook.200.<symbol>` — 200 (T45).
    /// Признак берётся из `topic` самим разбором (`bybit::ws`), а не из
    /// конфигурации соединения: один сокет несёт оба потока по каждому
    /// инструменту, и книга/файл выбираются этим полем. У записи на диске
    /// (формат v3, В-49) этого признака нет — он определяется только живым
    /// топиком; реплей несёт глубину основного файла
    /// (`bybit::conn::ORDERBOOK_DEPTH`).
    pub depth: u32,
    pub u: u64,
