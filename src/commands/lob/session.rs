    ...
    loop {
        ...
        match event {
            ...
            Event::Gap {
                ...
                // только счётчик эпизодов и максимум длительности. Событие
                // приходит на каждом тике пинга, пока эпизод не кончился
                // (`bybit::conn::Connection::run_with_backoff`) — счётчик
                // растёт только на первом (`first_of_episode`), максимум
