...
let SEL = null;
const FLT = {eaten:true,pulled:true,mixed:true,alive:true,minLife:0,topShare:0.2};
...
function coinHtml(co){
  ...
  h+='<h3>Картина за последний час записи</h3><div class="card">'+
    ...
    '<div class="ctl"><label><input type="checkbox" id="f_eaten"'+(FLT.eaten?' checked':'')+'> проели</label><label><input type="checkbox" id="f_pulled"'+(FLT.pulled?' checked':'')+'> сняли</label><label><input type="checkbox" id="f_mixed"'+(FLT.mixed?' checked':'')+'> смешанно</label><label><input type="checkbox" id="f_alive"'+(FLT.alive?' checked':'')+'> живые</label>'+
    ...
    '<div class="note">окно '+D.picture_minutes+' мин до последнего кадра ('+hhmm(co.picture.to_ms)+' UTC); диапазон цены — середина окна ± 25 bps (дальше не входит ни в одну корзину расстояния); за диапазоном осталось полосок: '+co.picture.clipped+(co.picture.bars_total>co.picture.bars.length?'; в окне '+co.picture.bars_total+' полосок, нарисованы '+co.picture.bars.length+' самых крупных':'')+'</div></div>';
...
function drawPicture(co){
  for (const k of ['eaten','pulled','mixed','alive']) document.getElementById('f_'+k).onchange = e => {FLT[k]=e.target.checked; drawPicture(co);};
  ...
  svg.innerHTML=s;
  document.getElementById('pic_n').textContent='показано полосок: '+n+' из '+P.bars.length;
}
