...
/// Пул одного инструмента с полом `h3_lots = 5` — как у фикстур `profiles`.
...
/// Четыре касания — обе стороны, оба исхода, все с уровнем, жившим до
/// кадра (В-43). Бид 100 снят на 1000 мс → бид 99 стал лучшим (касание 0,
/// база — середина 102 как есть); на 2000 мс 100 родился заново лучшей
/// ценой (не касание) → 99 ушёл с лучшей цены: **отскок**, середина к 1 с
/// поднялась до 102.5 — известный сдвиг `+1 тик / 204` = +49.0196 bps «в
/// сторону отскока». Аск 105 снят на 3000 мс → аск 106 лучший (касание 0),
/// на 4000 мс 105 вернулся → **отскок** аска, середина упала 103 → 102.5:
/// +48.5437 bps. На 5000 мс 100 снят снова → 99 лучший второй раз (индекс
/// 1), на 6000 мс 99 усох до лота — **смерть в касании**; на 7000 мс 105
/// снят → 106 лучший второй раз, на 8000 мс усох — **смерть**. Бид 98 и
/// аск 107 касаний не дают.
...
/// сутки без `verify ok` не читаются (`n_days = 2`, не 3). Критерий 3:
/// `runs.csv` получает по строке на id — корзины + крест, повторный прогон
/// дописывает вторую партию.
#[test]
fn two_verified_days_fill_marginals_and_cross_and_the_unverified_day_is_not_read() {
    ...
    for scope in ["pool", SYMBOL] {
        ...
        assert_eq!(n(&touch_marginal_id(scope, "duration", "[1s,10s)")), 8);
    ...
    assert_eq!(
        cell(&header, bid, "level_hours_utc"),
        "0",
    ...
    assert!(
        lines[0].contains(" alpha=0.005 replications=9999 seed=0"),
        "{}",
        lines[0]
    );
    ...
    // Журнал испытаний: строка на id, все подтверждающие, повтор — вторая партия.
    let runs = read_run_rows(&args.runs_out).unwrap();
    assert_eq!(runs.len(), touch_grid_size(1));
    ...
    let ids: Vec<String> = runs
        ...
        .collect();
    assert_eq!(ids, touch_profile_grid(&[SYMBOL.to_string()]));
...
/// Критерий приёмки 2: известный сдвиг середины после касания — отскок
/// бида 99 с базы 102 (как есть на `start_ms`, В-43) к 102.5 через 1 с:
/// `m_1000ms = +1 тик / 204 × 10⁴ = 49.0196 bps`; интервал по двум
/// суткам-кластерам держит точку внутри; `n_days = 2`. На 100 мс сдвига
/// ещё нет — ноль, не `-0.0`.
#[test]
fn interval_brackets_the_known_mid_shift_over_two_day_clusters() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    three_day_root(root);
    let out = root.join("touch-profiles.csv");
    run_touch_profiles(&base_args(root, out.clone())).unwrap();
    let (header, rows) = read_table(&out);
    ...
    assert_eq!(cell(&header, bid, "n_days"), "2");
    ...
    assert_eq!(
        cell(&header, bounced, "m_100ms"),
        "0.000000",
        "через 100 мс среза нет"
    );
    assert_eq!(cell(&header, bounced, "m_100ms_lower"), "0.000000");
    assert_eq!(
        cell(&header, bounced, "m_100ms_upper"),
        "0.000000",
        "не -0.000000"
    );
...
fn root_that_is_itself_a_session_directory_is_read_as_one_session() {
    ...
    let root = dir.path();
    write_instruments_csv(root, &[(SYMBOL, 5)]);
    write_session_dir(root, "2026-09-12", true, &four_touch_frames());
    let out = root.join("touch-profiles.csv");
    let summary = run_touch_profiles(&base_args(root, out.clone())).unwrap();
...
/// `--allow-unverified`: сутки без маркера читаются, шапка несёт `debug`,
/// `runs.csv` не пишется (данными не является).
#[test]
fn allow_unverified_reads_unmarked_days_marks_debug_and_skips_runs_csv() {
    ...
    let summary = run_touch_profiles(&args).unwrap();
    assert!(summary.debug);
    assert_eq!(summary.touches, 12);
    assert_eq!(summary.trials, 0);
    assert!(!args.runs_out.exists(), "отладка журнал не трогает");
    let (header, rows) = read_table(&out);
    let bid = rows.get(&touch_marginal_id("pool", "side", "bid")).unwrap();
    assert_eq!(cell(&header, bid, "n_days"), "3");
    assert!(header_lines(&out)[0].ends_with(" debug"));
}

/// `--preregistration`: окно — тот же файл, что у `profiles`; сутки вне
/// окна в таблицу не входят, шапка печатает `format_window_line`.
#[test]
fn preregistered_window_excludes_days_outside_it() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    three_day_root(root);
    let prereg = root.join("preregistration.md");
    std::fs::write(
        &prereg,
        "exploratory: 2026-09-01\nconfirmatory: 2026-09-01\n",
    )
    .unwrap();
    let out = root.join("touch-profiles.csv");
    let mut args = base_args(root, out.clone());
    args.preregistration = Some(prereg);
    let summary = run_touch_profiles(&args).unwrap();
