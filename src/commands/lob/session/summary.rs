...
pub struct SessionSummary {
    ...
    /// Свободное место диска записи в байтах на момент этой записи файла
    /// (A1, 2026-09-17) — замер `statvfs` на тике сессии, вне событийного
    /// пути. `None` — замер недоступен (не-Unix) либо путь не отвечает.
    #[serde(default)]
    pub disk_free_bytes: Option<u64>,
    /// Порог сторожа диска в байтах, с которым сравнивалось `disk_free_bytes`
    /// (`--disk-warn-gib`) — чтобы дежурный видел не только число, но и
    /// правило, по которому оно названо тревогой.
    #[serde(default)]
    pub disk_warn_bytes: Option<u64>,
    /// `disk_free_bytes < disk_warn_bytes` на момент записи файла: одна
    /// строка stderr на переход в это состояние и признак здесь. Полный диск
    /// гасит запись всех монет сразу, и `gaps.csv` тоже (аудит V2/V6) — до
    /// аварии это видно только тут и в журнале юнита.
    #[serde(default)]
    pub disk_free_low: bool,
}
