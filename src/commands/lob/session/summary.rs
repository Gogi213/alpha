...
pub struct SessionSummary {
    ...
    /// `bybit::conn` шлёт обновление на каждом тике пинга, пока эпизод не
    /// кончился, поэтому число отражает фактическую длину, а не только
    /// значение первого срабатывания (≈`recv_timeout`), и учитывает эпизод,
    /// ещё идущий на момент записи этой сводки. `None`, пока эпизодов не
    /// было — считать ноль без единого замера значило бы изобретённое
    /// число, а не измеренное (правило 1 `interfaces.md`, тот же приём, что
    /// `parse_p99_ns`).
    #[serde(default)]
