...
pub(crate) use replay::{
    ...
    replay_symbol_over_configs_keep, replay_symbol_touches_and_second_mids, trade_hit_from_record,
    ReplayDay, ReplayKeep,
};
...
pub enum LobCommand {
    ...
    // `Box`: `--carry-root` добавил `BounceGridArgs` достаточно байт, чтобы
    // `LobCommand` не проходил `clippy::large_enum_variant` — сама структура
    // не тронута, коробка только здесь (clap 4 разбирает `Box<T: Args>`
    // напрямую, тот же приём, что рекомендует сам клиппи).
    BounceGrid(Box<BounceGridArgs>),
