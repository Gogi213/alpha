...
pub enum GapKind {
    ...
    /// символа нет в листинге, он переименован или поток не отдаётся. Шов
    /// покрытия: данных у инструмента не будет вовсе, но запись честная
    /// (после отказа инструмент получает снапшот, как только подписка
    /// состоится). До этой правки отказ не покидал `Connection::run` вовсе —
    /// `{"success":false}` читался как служебное сообщение, и инструмент
    /// молча стоял без данных.
    SubscribeFailed,
}
