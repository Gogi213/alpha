        ...
        tokio::select! {
            msg = rx.recv() => {
                ...
                match conn_event {
                    ...
                    // V5, доработка 2026-09-24: тишина рынка (решение
                    // владельца — соединение по ней не рвётся) уже дала
                    // предупреждение в stderr внутри `bybit::conn` — этот
                    // рекордер не ведёт `session.json` со счётчиком эпизодов,
                    // считать здесь не во что, а строки `gaps.csv` тишина не
                    // даёт (соединение живо, покрытие не потеряно).
