    ...
    B: Bot<MD>,
{
    ...
    match state.phase {
        ...
        Phase::Idle => {
            ...
            // не вольность: чем заменить отвергнутый пост-онли, решает
            // владелец (В-44 писался до данных).
            // Вход — обычный лимит (`GTC`), а не пост-онли. Находка прогона
            // T38 2026-09-13: `GTX` на цене «за тик перед плотностью»
            // отвергается биржей почти всегда (в момент касания спред уже
            // сжался, и такая заявка пересекает) — 10 исполнений на 4520
            // касаний ZEC против 333 у `GTC`. Пост-онли вход оставлен
            // выключенным осознанно: неисполнимый план не даёт ответа об
            // альфе. Что правильнее — решает владелец (В-44 писался до
            // данных).
            ...
            let entry_wait = matches!(state.plan, TradePlan::Bounce { .. });
            match side {
                HbtSide::Buy => {
                    bot.submit_buy_order(
                        state.asset_no,
