...
/// сутки приходят из ротации по UTC, а не из рук пользователя. `pub` с
/// таска 23: той же проверкой `commands::lob::session_days_in_dir` отсеивает
/// имена файлов, у которых хвост не сутки.
pub fn day_format_ok(day: &str) -> bool {
    let b = day.as_bytes();
    b.len() == 10
        && b.get(4) == Some(&b'-')
        && b.get(7) == Some(&b'-')
