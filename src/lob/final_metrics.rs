...
//! ещё нет) и 7.2 (`stats`: там есть только wild cluster bootstrap-t, готовых
//! оценщиков DSR/PBO/CPCV нет — этот модуль их не дублирует из `stats`, а
//! вводит здесь, потому что в `stats` их нет вовсе).
//!
...
//!
//! Число испытаний для DSR/PBO считает журнал `runs.csv` шага 7.1: пилотные
//! прогоны обоих кандидатов и каждый перезапуск после шага 5 — тоже испытания
//! (OPEN_QUESTIONS H10, раздел Failure). Пока 7.1 нет, чтение журнала —
//! помеченная заглушка `trials_from_runs_csv_placeholder`, формат журнала
//! здесь не выдумывается.
//!
...
// ---------------------------------------------------------------------------
// Вход от бэктеста, сводный отчёт, заглушка runs.csv.
// ---------------------------------------------------------------------------
...
///
/// ЗАГЛУШКА: шаг 7.1 ещё не реализован (тикет pending), поэтому функция всегда
/// возвращает `None` — «журнал недоступен», а не ноль испытаний. Формат
/// журнала здесь намеренно не выдумывается: когда 7.1 задаст схему строк
/// (прогон на строку, включая пилоты и перезапуски), тело начнёт считать
/// строки этого пути. Вызывающий код обязан проверить `None` до подстановки
/// в DSR/PBO, а не подменять его единицей.
pub fn trials_from_runs_csv_placeholder(_path: &Path) -> Option<usize> {
    None
}
...
mod tests {
    ...
    /// Заглушка честно сообщает об отсутствии 7.1: всегда `None`, формат
    /// журнала не выдуман — проверять вызывающему коду, а не числу.
    #[test]
    fn runs_csv_reader_is_a_marked_placeholder_until_7_1() {
        assert_eq!(
            trials_from_runs_csv_placeholder(std::path::Path::new("docs/plan/runs.csv")),
            None
        );
    }
}
