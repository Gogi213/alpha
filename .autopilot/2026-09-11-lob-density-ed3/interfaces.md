...
- `lob::runs::log_touch_profile_trials(path, ts_utc, ids)` — по строке `RunKind::Confirmatory` с деталью `touch_profile <id>` на каждый id сетки (тот же учёт, что `shortlist::log_profile_trials`, отличим по префиксу); DSR/`trials_from_runs_csv` считает их строками. Боевой прогон пишет всю сетку каждый раз (журнал, не перезапись).
