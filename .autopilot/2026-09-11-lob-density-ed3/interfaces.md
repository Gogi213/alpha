...
## Из таска 35 — касания живых уровней (R90, В-42)

- `lob::levels::TouchRecord { side, price_tick, touch_index: u32, start_ms, end_ms, duration_ms, level_birth_ms, size_at_touch, size_max_before, traded_during, frontrun_lots, round_zeros: u8, ended_by_death: bool, stack_levels: u32 }` + `age_ms()`; `round_zeros(price_tick) -> u8` (нули в конце десятичной записи тика, потолок `ROUND_ZEROS_CAP = 3`). Определения — doc-раздел «Касания» в `levels.rs`: начало — уровень на индексе 0 кадра своей стороны (кадр идёт от лучшей цены вглубь — тот же контракт, что `in_top50 = i < 50`; `LevelObs` не расширялся), конец — индекс > 0 или смерть (`ended_by_death`, `end_ms = death_ms`; смерть и уход в одном кадре — смерть); уровень, родившийся лучшей ценой, касается с рождения (фронтран 0, `size_max_before` = размер рождения); `frontrun_lots` — префикс лотов до индекса уровня на последнем кадре, где он наблюдался до касания; `stack_levels` — выжившие свип той же стороны с `seen_size ≥ H3` после кадра начала (сам уровень входит); касания прогревных уровней считаются, не эмитируются.
- `LevelTracker::observe_frame_with_touches(ts_ms, side, levels, out, touches: &mut Vec<TouchRecord>)` — полный кадр; `observe_frame(ts_ms, side, levels, out)` — прежняя сигнатура, обёртка с внутренним буфером `touch_scratch` (`mem::take` без выделения), касания отбрасываются — её зовут `watch.rs::feed_session_frame`, `replay.rs::feed_frames` (→ `profiles/sessions.rs`). Состояние — `Live { better_lots, touch_index, touch: Option<Touch> }`, `Touch { start_ms, start_frame, size_at, size_max_before, frontrun, traded_at_start, stack, end_pending }`; буфер `touched: Vec<(u8, i64)>` ключей с событием касания за кадр. Порядок: касания умерших — в свипе перед их `LevelRecord` (по ключу), касания выживших — по порядку кадра. Тест GC `touching_frames_allocate_nothing`: начало+конец касания на каждом кадре, сделки внутри, обе сигнатуры — ноль аллокаций. `LevelRecord`/правило 70/20/`repeat_count`/`repriced`/`LiveLevel` не менялись — прежние тесты без правок.
- `lob::markout`: `APPROACH_MS = [HORIZONS_MS[1], HORIZONS_MS[2]]`; `touch_markout_bps(side, base2x, fut2x)` — знак «в сторону отскока» (бид `s = +1`, аск `s = -1`, ровно минус `markout_bps`; знак в целых до деления — нулевой сдвиг не даёт `-0.0`); `markouts_for_touch(&TouchRecord, mids) -> [Option<f64>; 4]` (база — `base_before(mids, start_ms)`); `approach_bps(side, mids, start_ms, back_ms)` — сдвиг середины от среза как есть на `t0 − back_ms` до базы `t0`, знак «к уровню» (совпадает со знаком `markout_bps`, формула та же с началом окна в роли базы); `approaches_for_touch -> [Option<f64>; 2]`. `markout_bps`/`markouts_for_level` не менялись.
...
- `lob touches --root --symbol --h3-mode floor|percentile [--h3-lots] [--h3-k] [--warmup-ms] [--repeat-window-ms] [--out]` (`commands/lob/touches.rs`, `TouchesArgs`/`run_touches`/`TouchesSummary { days, touches, out }`, stdout `touches: days= touches= out=`) → `<root>/touches-<SYMBOL>.csv`, колонки `TOUCHES_COLUMNS` (23): `day_utc, side, price_tick, touch_index, start_ms, end_ms, duration_ms, level_birth_ms, age_ms, size_at_touch, size_max_before, traded_during, frontrun_lots, round_zeros, ended_by_death, stack_levels, dist_bps, m_100ms, m_1s, m_10s, m_60s, approach_1s, approach_10s`; `dist_bps` — тот же расчёт, что `profiles::axes::distance_bps_at_birth` (от полей касания; `profiles/` не тронут); пустая ячейка — нет среза, не ноль. Тесты: `touches/tests.rs::touches_fixture_writes_touch_rows_with_expected_columns` (4 касания, значения по строкам), `record/tests.rs` и `lob/tests.rs::lob_help_lists_all_subcommands` — новая подкоманда в перечислениях (18).
- Живая проверка: `lob touches --root data/session-debug/t34 --symbol SOLUSDT --h3-mode floor` (`h3_lots = 9`) → 72 касания за 5 мин: 53 смертью, 6 повторных (`touch_index = 1`), 18 с фронтраном, медиана длительности 1.3 с, `stack_levels` 48–50 (при поле 9 лотов почти весь топ-50 — уровни: следствие заглушки `k = 1.0`, В-30), `m_10s` среднее −0.2 bps (n = 70), `approach_1s` +0.3 bps.
