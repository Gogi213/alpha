...
- Документы: `docs/archive/plan-2026-09-11/REQUIREMENTS.md` переписан; `SETTLED.md` ПЛАН-2 — эррата в строке; `docs/archive/plan-2026-09-11/` — `OPEN_QUESTIONS.md`, `critique-*.json`, `README.md`.
...
- Замер «до → после» — `docs/archive/findings/collector-2026-09-12.md`: разбор p99 бенч снапшот 116 → 49 мкс; аллокаций/сообщение 328 → 8 (снапшот), 38 → 2 (дельта), 93 → 0 (лента); живой пул 8×5 мин: `parse_p99_ns` 249 → 104 мкс (бюджет 200 — впервые в бюджете), `queue_p99_ns` 670 → 299, RSS плоский, CPU 2.9 → 1.7 %, байт/запись 15.6 → 7.3 (−53 %).
...
- Замер (`docs/archive/findings/collector-2026-09-12.md`, «761»): 8 → 761: CPU 2.0 → 24.2 % (0.253 → 0.032 %/инстр.), RSS 17.8 → 159.5 МиБ (215 КиБ/инстр.), 6.9 КБ/мин/инстр., байт/запись 6.38, parse p99 87 мкс, queue p99 27.8 мс (долг живого бота), 7.75 ГБ/сутки.
...
## Из таска 35 — касания живых уровней (R90, В-42, ревью В-43)

- `lob::levels::TouchRecord { side, price_tick, touch_index: u32, start_ms, end_ms, duration_ms, level_birth_ms, size_at_touch, size_max_before, traded_during, frontrun_lots, round_zeros: u8, ended_by_death: bool, stack_levels: u32 }` + `age_ms()`; `round_zeros(price_tick) -> u8` (нули в конце десятичной записи тика, потолок `ROUND_ZEROS_CAP = 3`). Определения — doc-раздел «Касания» в `levels.rs`: касание **начинается** у уровня, **жившего до кадра** (`birth_ms < ts_ms`, В-43), который **стал** лучшей ценой своей стороны — индекс 0 кадра, не будучи им на прошлом наблюдении (`Live.was_best`); кадр идёт от лучшей цены вглубь — тот же контракт, что `in_top50 = i < 50`, `LevelObs` не расширялся. **Рождение лучшей ценой — не касание** (и стать лучшим в миллисекунду рождения — тоже): такой уровень касается при следующем становлении лучшим после ухода. Конец — индекс > 0 или смерть (`ended_by_death`, `end_ms = death_ms`; смерть и уход в одном кадре — смерть, В-43 (3)); `frontrun_lots` — префикс лотов до индекса уровня на последнем кадре, где он наблюдался до касания; `size_max_before` — максимум до кадра начала; `stack_levels` — выжившие свип той же стороны с `seen_size ≥ H3` после кадра начала (сам уровень входит; при отладочном пороге вырожден — свойство порога); проверки размера при старте нет — `size_at_touch` в записи, фильтр в анализе (касание нулевой длительности со смертью в том же кадре — законная строка); касания прогревных уровней считаются, не эмитируются.
- `LevelTracker::observe_frame_with_touches(ts_ms, side, levels, out, touches: &mut Vec<TouchRecord>)` — полный кадр; `observe_frame(ts_ms, side, levels, out)` — прежняя сигнатура, обёртка с внутренним буфером `touch_scratch` (`mem::take` без выделения), касания отбрасываются — её зовут `watch.rs::feed_session_frame`, `replay.rs::feed_frames` (→ `profiles/sessions.rs`). Состояние — `Live { better_lots, was_best, touch_index, touch: Option<Touch> }`, `Touch { start_ms, start_frame, size_at, size_max_before, frontrun, traded_at_start, stack, end_pending }`; буфер `touched: Vec<(u8, i64)>` ключей с событием касания за кадр. Порядок: касания умерших — в свипе перед их `LevelRecord` (по ключу), касания выживших — по порядку кадра. Тест GC `touching_frames_allocate_nothing`: начало+конец касания на каждом кадре, сделки внутри, обе сигнатуры — ноль аллокаций. `LevelRecord`/правило 70/20/`repeat_count`/`repriced`/`LiveLevel` не менялись — прежние тесты без правок.
- `lob::markout`: `signed_return_bps(sigma, base2x, fut2x)` — одна приватная арифметика под `markout_bps` (бид −1/аск +1) и `touch_markout_bps` (бид +1/аск −1 — «в сторону отскока», ровно минус смерти; знак в целых до деления — нулевой сдвиг `+0.0`); `touch_base(mids, start_ms) -> Option<(ts, mid2x)>` — **срез как есть на `start_ms`** (`sample_asof(mids, start_ms, 0)`, В-43: середина уже с уровнем на лучшей цене; «строго до» несло шаг кадра касания как −1 тик на всех горизонтах); `markouts_for_touch(&TouchRecord, mids) -> [Option<f64>; 4]` — горизонты `HORIZONS_MS` от метки базы; `APPROACH_MS = [HORIZONS_MS[1], HORIZONS_MS[2]]`; `approach_bps(side, mids, start_ms, back_ms)` — от среза как есть на `start_ms − back_ms` до той же базы, знак «к уровню» (совпадает со знаком `markout_bps`, формула та же с началом окна в роли базы); `approaches_for_touch -> [Option<f64>; 2]`; `distance_bps_at_birth(mids, birth_ms, price_tick)` — расстояние при рождении (`base_before(birth_ms + 1)`, модуль), `profiles/axes.rs::distance_bps_at_birth(mids, &LevelRecord)` делегирует ему одной строкой (единственная правка в `profiles/`). `markout_bps`/`markouts_for_level`/`base_before` не менялись. Тест базы: срез ровно на `start_ms` со сдвигом на уровень → `m == 0` на всех горизонтах, прежняя база дала бы < 0.
- `replay.rs`: `ReplayDay.touches: Vec<TouchRecord>`, `DayWork.touches`, `feed_frames_multi(.., out, touches, mids)` зовёт `observe_frame_with_touches`; `feed_frames` (одиночный, для `profiles/sessions.rs`) сигнатуру не менял — касаний не собирает.
- `lob touches --root --symbol --h3-mode floor|percentile [--h3-lots] [--h3-k] [--warmup-ms] [--repeat-window-ms] [--out]` (`commands/lob/touches.rs`, `TouchesArgs`/`run_touches`/`TouchesSummary { days, touches, out }`, stdout `touches: days= touches= out=`) → `<root>/touches-<SYMBOL>.csv`, колонки `TOUCHES_COLUMNS: [&str; TOUCHES_WIDTH = 23]`, строка — `[String; TOUCHES_WIDTH]` (один источник арности, расхождение не компилируется): `day_utc, side, price_tick, touch_index, start_ms, end_ms, duration_ms, birth_ms` (как в `levels-*`/`markout-*`, для джойна), `age_ms, size_at_touch, size_max_before, traded_during, frontrun_lots, round_zeros, ended_by_death, stack_levels, dist_bps, m_100ms, m_1s, m_10s, m_60s, approach_1s, approach_10s`; пустая ячейка — нет среза, не ноль. Тесты: `touches/tests.rs::touches_fixture_writes_touch_rows_with_expected_columns` (3 касания одного уровня 99 — индексы 0/1/2, фронтран 10, `m_100ms = 0` при базе как есть, смерть внутри третьего; родившиеся лучшими 100/101/105 строк не дают), `record/tests.rs` и `lob/tests.rs::lob_help_lists_all_subcommands` — новая подкоманда в перечислениях (18).
- Живая проверка после В-43: `lob touches --root data/session-debug/t34 --symbol SOLUSDT --h3-mode floor` (`h3_lots = 9`) → **18** касаний за 5 мин (было 72 до В-43: 54 — рождения лучшей ценой): 15 смертью, 2 повторных (`touch_index = 1`), 18/18 с фронтраном, возраст ≥ 61 мс, медиана длительности 311 мс, `stack_levels` 48–50 (пол 9 лотов — почти весь топ-50 уровни, заглушка `k = 1.0`); `m_100ms` = 0 у 14 из 18 (среднее −0.14 bps), `m_10s` среднее −0.19 bps, `approach_1s` +1.1 bps, `approach_10s` +1.2 bps (n = 18) — сдвиг базы «−1 тик на всех горизонтах» ушёл. `m_*` из CSV до В-43 результатом не считаются.
- Не в этом таске: дашборд, оси профилей/шортлиста по касаниям (после предрегистрации отдельным решением), `LevelObs.best`/индекс (не понадобился — индекс кадра).
- **Тикет 35b (В-45).** `TouchRecord.frontrun_lots` — лоты той же стороны впереди уровня на кадре **за секунду до касания**: два слота `FrontrunSlot { start_ms, first_lots, last_ms, last_lots }` в `Live` (`slot_new`/`slot_old`) шагом `levels::FRONTRUN_BACK_MS = HORIZONS_MS[1]` от первого наблюдения слота (`Live::observe_frontrun`); на старте касания `Live::frontrun_before(ts)` — последнее наблюдение старого слота, если оно не позже `ts − 1 с`, иначе его первое (оно не позже `ts − 1 с` по построению шага); старого нет — первое наблюдение нового (кадр рождения). Гарантия: кадр в 1–2 с до касания (плюс интервал между кадрами), **никогда** позже секунды до касания — doc модуля «Касания». Прежняя величина — новое поле `swept_lots` (последний кадр до касания, `Live.better_lots`). `stack_levels` — уровни ≥ `H3` той же стороны **в окне `shortlist::DISTANCE_MAX_BPS` (25 bps)** от цены уровня, включая его: `levels::stack_window_ticks(price_tick) = price_tick × 25 / 10 000` целочисленно (0 тиков при цене < 400 тиков — только сам уровень), один раз на старте в `Touch.window_ticks`; счёт — `BTreeMap::range` по выжившим свип (буфер `touched: Vec<Touched { key, stack }>`). `DISTANCE_MAX_BPS: i64 = 25` объявлена рядом с `DISTANCE_BOUNDS_BPS`, равенство проверяется `const` на компиляции — `f64` в `levels.rs` по-прежнему нет. `markout::within_touch(duration_ms) -> [bool; 4]` (`HORIZONS_MS[i] ≤ duration_ms`, включительно) и `markouts_for_touch_outside` (`None` на горизонтах внутри касания) — один источник правила для CSV/дашборда/T37. `lob touches` CSV: `TOUCHES_WIDTH = 28` — `+swept_lots` после `frontrun_lots`, `+within_touch_100ms/1s/10s/60s` после `m_60s`. Дашборд: `Touches += { stack_window_bps, k_stub (действующий k == dashboard::K_STUB = 1.0: --h3-k или колонка k instruments.csv, только floor), within_touch: [usize; 4] }`, средние `m_bps` строк — по `markouts_for_touch_outside`; `Mark += { sw: Option<доля сметённого>, w: bool (10 с внутри касания) }`; страница: фронтран «за секунду до касания», «сметено» в наведении, «k — заглушка (В-30)» у завала, счётчики исключённых ячеек. Тесты: `levels/tests.rs` +3, `markout/tests.rs` +1, `dashboard/tests.rs` +1 (ожидания `stack_levels` у тиков ~100 стали 1 — окно 0 тиков). Живая проверка t34 SOL: `frontrun = 0` у 2 из 18, `swept = 0` у 0, `stack_levels` 25–26 (было 48–50). Смерти уровней не менялись.

## Из таска 36 — касания на дашборде (R90, В-42/В-43/В-44)

- `lob::touch_axes` (новый, `src/lob/touch_axes.rs`) — **единственное место** корзин и границ осей касаний В-44; T37 берёт их отсюда. Константы с цитатой источника в doc-комментарии: `AGE_LABELS`/`AGE_BOUNDS_MS` (`[0,10m)`, `[10m,1h)`, `[1h,inf)`; D 01:57) + `age_bucket(age_ms) -> Option`; `FRONTRUN_LABELS` (`0`, `(0,0.5)`, `[0.5,inf)`; D 03:57) + `FRONTRUN_HALF = 0.5`, `frontrun_share(frontrun_lots, size_at_touch) -> Option<f64>` (знаменатель — `size_at_touch`, `None` при размере ≤ 0), `frontrun_bucket(share) -> Option` (ровно ноль — своя корзина); `ROUND_LABELS` (`0`, `1`, `>=2`; D 01:57, S 17:48) + `round_bucket(u8)` — тотальная; `TOUCH_INDEX_LABELS` (`1`, `2-3`, `>=4`; S 12:50) + `touch_index_bucket(u32)` — тотальная (0 → «1», 1–2 → «2-3», ≥ 3 → «>=4»); `APPROACH_LABELS`/`APPROACH_BOUNDS_BPS` (`<0`, `[0,1)`, `[1,2.5)`, `[2.5,inf)` — границы из `DISTANCE_BOUNDS_BPS[0..3]`, последняя открыта вверх; S 05:19) + `approach_bucket(bps) -> Option` (`NaN` → `None`); `TOUCH_OUTCOME_LABELS = ["bounced", "eaten"]` + `touch_outcome(ended_by_death)`. Сторона/размер/длительность корзин не дублируют: `SIDE_LABELS`, `SIZE_LABELS` + `profiles::size_bucket`, `LIFETIME_LABELS` + `profiles::lifetime_bucket` (В-44 называет их теми же метками). Тесты границ — `lob/touch_axes/tests.rs` (включительно/исключительно ровно по В-44).
- `lob dashboard`: `Coin.touches: Touches { total, bounced, eaten, share_bounced, per_hour, outcomes: Vec<TouchRow>, all, by_age, by_frontrun, by_round, by_index, by_approach, by_duration, by_side, by_size, cross_outcome_age: Vec<TouchCell> (2 × 3, `TOUCH_OUTCOME_LABELS` × `AGE_LABELS`), approach_missing, size_below_h3, approach_ms: APPROACH_MS, stack_median: Option<f64>, stack_shown }`; `TouchRow { label, n, share, share_bounced, m_bps: [Option<f64>; 4], m10s_n, m10s_bounced_bps, m10s_bounced_n }`; `TouchCell { outcome, age, n, share, m10s_bps, m10s_n }`. Источник — `ReplayDay.touches` того же `replay_symbol` (второго прохода нет), markout — `markouts_for_touch` (база как есть на `start_ms`, В-43), подход — `approaches_for_touch()[0]` (1 с). `stack_median` — `None` при `# debug` в `instruments.csv` (`stack_shown = false`, страница печатает «не показан: порог отладочный»); иначе печатается «N из 50». `size_below_h3` — касания с `size_at_touch < H3` (усохший уровень, В-43: фильтр в анализе) — не входят в корзины размера, число печатается; `approach_missing` — без среза за секунду до касания. Без касаний — блок с нулями и всеми метками, не ошибка (`build_touches(&[], ..)` и для `empty_coin`).
- Картина: `Picture.marks: Vec<Mark { t: start_ms, p, s, o: bounced|eaten, x: size_at_touch ×H3, a: age_ms, d: duration_ms, i: touch_index, fr: Option<доля>, ap: Option<подход 1 с>, m: Option<m_10s> }>`, `marks_total`, `marks_clipped`; окно и диапазон цены — те же, что у полосок, потолок — тот же `PICTURE_MAX_BARS` по `x`. Страница (`dashboard_page.html`): треугольник у `start_ms` на цене уровня (вверх — бид, вниз — аск), цвет `--bounced`/`--teaten`, `<title>` с числами, чекбокс `f_touch` (`FLT.touch`); блок «Касания: цена дошла до плотности» — `touchesHtml(co)` (итог + `touchOutcomeTable`, восемь `touchTable` по осям, `touchCrossTable`), плитка — «касаний за запись» и «отскочила»; словарь + касание, отскок / проели на касании, фронтран, подход, круглость, завал. Оговорки текстом: база В-43, «движение, не сделка», суток < `G_MIN`.
- Тесты: `dashboard/tests.rs::touches_block_counts_a_bounce_and_a_death_with_axes_cross_and_marks` (фикстура `touch_frames`: бид 9999 — касание 0 отскок 3 с с фронтраном 1.0, касание 1 смертью с фронтраном 0.3; `total = 2`, доли, все метки осей в порядке `touch_axes`, крест 6 клеток, две метки с числами наведения), `a_coin_without_touches_gets_a_zero_block_not_an_error`, `page_is_self_contained_html_without_external_urls` (+ строки `touchesHtml`/`P.marks`/`f_touch`/`cross_outcome_age` в шаблоне), round-trip JSON с касаниями; 6 тестов границ `touch_axes`. Всего 673 (было 665).
- Живая проверка `lob dashboard --root data/always-on/20260912T201737Z --out data/dashboard-check` (боевая десятка, `k = 1.0` заглушка, 1.47 ч записи; 13 с на расчёт десяти монет): SOL — 348 касаний (236 в час), отскочила 56 (16.1 %), проели на касании 292; `m` отскоков +0.40 / +0.95 / +1.25 / +1.18 bps на 100 мс / 1 с / 10 с / 60 с, проели −0.28 / −0.32 / −0.43 / −0.33, все −0.17 / −0.12 / −0.16 / −0.09; возраст — 345 в `[0,10m)`, 3 в `[10m,1h)`, 0 старше часа (1.5 ч записи — верх оси возраста пуст по длине записи); фронтран `(0,0.5)` 154 (отскок 24.0 %, `m_10s` отскоков +1.34) / `[0.5,inf)` 194 (9.8 %, +1.06), `0` — ни одного; круглость 0 — 316, 1 — 32 (отскок 6.2 %); номер касания 1-е 318 (отскок 14.8 %), 2–3-е 30 (30.0 %), 4-е+ 0; подход `<0` 38 / `[0,1)` 190 (13.2 %) / `[1,2.5)` 61 (21.3 %, `m_10s` отскоков +2.08) / `[2.5,inf)` 59; длительность `[0,1s)` 241 (12.4 %) / `[1s,10s)` 55 (25.5 %) / `[10s,inf)` 52 (23.1 %, `m_10s` отскоков 0.00); бид 175 / аск 173; размер `[4,inf)` 326 из 339 в корзинах, `size_below_h3` = 9; крест: отскок × `[0,10m)` 56 (+1.25), проели × `[0,10m)` 289 (−0.46), × `[10m,1h)` 3 (+2.30); завал медиана 49 из 50 (заглушка `k = 1.0` — почти весь топ-50 уровни, как в T35); картина — 260 меток из 260. Крайние доли фронтрана (до 5·10⁴ размера) — тот же порог: «плотность» в 9 лотов = 0.9 SOL. LSK — 35 579 касаний, меток 4000 из 24 143 (потолок). Наблюдение, не вердикт (суток 1 < G_MIN).
- Не в этом таске: оси касаний в профили/шортлист (T37), сделка-отскок в бэктесте (T38), горячий путь (`levels.rs`, `replay.rs`, `session.rs` не тронуты).

## Из таска 37 — профили касаний (R90, В-44)

- `lob touch-profiles --root <dir> --h3-mode floor|percentile [--h3-lots] [--h3-k] [--warmup-ms] [--repeat-window-ms] [--allow-unverified] [--out] [--now-utc] [--runs-out=docs/plan/runs.csv] [--preregistration] [--window-end]` (`commands/lob/touch_profiles.rs`, `TouchProfilesArgs`/`run_touch_profiles`/`TouchProfilesSummary { rows, touches, trials, out, debug }`, stdout `touch-profiles: rows= touches= trials= debug= out=`) → `docs/findings/touch-profiles-<дата>.csv`. Шапка: `# lob touch-profiles: h3_mode=..[ h3_k=..] warmup_ms=.. repeat_window_ms=.. alpha=.. replications=.. seed=..[ debug]`, вторая строка — окно: `format_window_line` при `--preregistration` (тот же файл и `load_or_write_window`, что у `profiles`/`shortlist`; сутки вне окна отсеиваются после реплея — `replay_symbol` читает каталог целиком), без него — `window: not preregistered <first>..<last> days=N sessions=K` (не тихое «все сессии»; на одних сутках `split_calendar` окно не построит, поэтому флаг необязателен). `rtt=` не печатается — RTT в markout не входит. Колонки `HEADER: [&str; 17]`: `profile_id, n, share_bounced, {m, m_lower, m_upper}×{100ms,1000ms,10000ms,60000ms}, n_days, level_hours_utc` (часы UTC **начала касания** — момент наблюдения, как час рождения у смертей); `none` в числах при `n = 0`; строка на каждый id сетки, включая `n = 0`.
- `--root` — корень с `instruments.csv` пула и подкаталогами сессий (раскладка `profiles`) **или** сам каталог сессии (в нём `session.json`; `data/always-on/<ts>`, `data/session-debug/<id>`), тогда сессия одна и пул — из его `instruments.csv`. Сессия без `session_parts_for` — молчаливый пропуск; маркер `verify-<SYMBOL>.status == ok` обязателен (fail-closed), `--allow-unverified` → метка `debug`, `runs.csv` не пишется. Реплей — `replay_symbol` (тот же, что `levels`/`touches`/`dashboard`; `ReplayDay.touches`), кластер — сутки, один индекс суток на весь прогон (общие сутки разных инструментов — один кластер для строк `pool`).
- `lob::touch_axes` += сетка: `TOUCH_AXES: [(&str, &[&str]); 9]` — `side`/`size`/`age`/`frontrun`/`round`/`index`/`approach`/`duration`/`outcome` с их метками (сторона/размер/длительность — `shortlist::SIDE_LABELS`/`SIZE_LABELS`/`LIFETIME_LABELS`); `POOL_SCOPE = "pool"`; `touch_marginal_id(scope, axis, label) = "<scope>:marginal:<axis>=<label>"`, `touch_cross_id(scope, outcome, age) = "<scope>:cross:<outcome>|<age>"`; `touch_profile_grid(&[symbol]) -> Vec<String>` — на область `pool` и на каждый инструмент: 26 маргиналов + 6 клеток креста, сортировано, без повторов; `touch_grid_size(n) = (26 + 6) × (n + 1)` из длин массивов (352 на десятку). Тест `touch_profile_grid_has_marginals_and_the_outcome_age_cross_per_scope`.
- Корзины касания в таблице — те же функции, что у дашборда (`age_bucket(age_ms())`, `frontrun_share(..).and_then(frontrun_bucket)`, `round_bucket`, `touch_index_bucket`, `approaches_for_touch()[0].and_then(approach_bucket)`, `profiles::lifetime_bucket(duration_ms)`, `profiles::size_bucket(size_at_touch / H3)`, `touch_outcome`); касание без корзины (размер ниже `H3`, нет среза подхода) в этой оси отсутствует, в остальных и в кресте — есть. Интервал `m` — `costs::net_fill_interval` с `filled = true` (тот же приём, что `m_lower` у `profiles`; `alpha`/`replications` — `stats::GATE_ALPHA`/`BOOTSTRAP_REPLICATIONS`, seed — `profiles::BOOTSTRAP_SEED`, теперь `pub(crate)`), верхняя граница — та же функция на наблюдениях с обратным знаком, взятая с минусом (`negate` без `-0.0`) — второго бутстрапа нет. `stats::wild_cluster_bootstrap_t` здесь не используется: он даёт p-значение, не интервал, и отказывает при `G < 7`.
- `lob::runs::log_touch_profile_trials(path, ts_utc, ids)` — по строке `RunKind::Confirmatory` с деталью `touch_profile <id>` на каждый id сетки (тот же учёт, что `shortlist::log_profile_trials`, отличим по префиксу); DSR/`trials_from_runs_csv` считает их строками. Боевой прогон писал всю сетку каждый раз — **снято ревью** (см. «Ревью T37» ниже: только `n > 0`, писатель `runs::log_trials`).
- Видимость (единственные правки в `profiles/`): `read_pool_symbols`, `read_verify_marker`, `session_dirs`, `distinct_session_days`, `hour_utc_of_ms`, `h3_mode_label` — `pub(super)` → `pub(crate)` с ре-экспортом из `profiles.rs`; `BOOTSTRAP_SEED` — `pub(crate)`. `--candidates-csv` у `touch-profiles` нет: покрытие книги ни в одну ось касаний не входит.
- Тесты `touch_profiles/tests.rs` (фикстура `four_touch_frames`: бид 99 и аск 106 — отскок первым касанием и смерть вторым, обе стороны, известный сдвиг +1 тик/204 = 49.0196 bps на 1 с): `two_verified_days_fill_marginals_and_cross_and_the_unverified_day_is_not_read` (64 строки, `n` по корзинам, тотальные оси и крест суммируются в 8, `n_days = 2` при третьих сутках без маркера, 64 строки `runs.csv` и 128 после повтора), `interval_brackets_the_known_mid_shift_over_two_day_clusters` (`lower < m < upper`, `m_100ms` = `0.000000` не `-0.000000`, байт-в-байт детерминизм), `root_that_is_itself_a_session_directory_is_read_as_one_session`, `allow_unverified_reads_unmarked_days_marks_debug_and_skips_runs_csv`, `preregistered_window_excludes_days_outside_it`; `lob_help_lists_all_subcommands` (19), `record/tests.rs` match. Всего 680 (было 673).
- Живая проверка `lob touch-profiles --root data/session-debug/t34 --h3-mode floor` (ZEC `verify ok`, 5 мин, `h3_lots = 4`; журнал — во времянку, `docs/plan/runs.csv` не тронут: сессия отладочная): 128 строк (пул + SOL/XRP/ZEC; SOL/XRP без маркера — нули), 75 касаний ZEC, 16 отскочили (21.3 %), `m_10s` отскоков +3.07 bps / проели −1.02 / все −0.15; все возрастом `[0,10m)` и первым касанием (5 мин записи); фронтран `(0,0.5)` 16 (отскок 37.5 %, `m_10s` +0.26) / `[0.5,inf)` 59 (16.9 %, −0.26); подход `[0,1)` 29 (27.6 %, +0.73) / `[1,2.5)` 28 / `[2.5,inf)` 9 (0 %, −1.74) / `<0` 7; длительность `[1s,10s)` 19 (42.1 %, +1.89) / `[0,1s)` 56 (14.3 %, −0.84); размер `[4,inf)` 55 из 60 (15 ниже `H3`); круглость 1 — 11 (36.4 %). Интервалы на одних сутках вырождены (один кластер — `lower = 0.5·m`, `upper = 1.5·m` по весам Уэбба) — наблюдение, не вердикт (суток 1 < G_MIN).
- Не в этом таске: сделка-отскок в бэктесте (T38), шортлист по касаниям, горячий путь (`levels.rs`, `replay.rs`, `session.rs` не тронуты).
- **Ревью T37 (2026-09-13), правки поверх.** (a) Боевой режим **требует `--preregistration`** — без него `bail!` той же строкой, что у `profiles` (R57); `--allow-unverified` → окно `None` (файл не читается), `debug`, `trials=0`, `runs.csv` не пишется. (b) Средние и интервалы — по `markout::markouts_for_touch_outside` (горизонт внутри касания — не наблюдение, В-45 (2)). (c) **Испытания — только id с `n > 0`** (В-18): непрочитанный инструмент пула даёт нули по своей области и в журнал не идёт; шапка `trials=N`, `TouchProfilesSummary.trials` — тот же счёт (0 в отладке). (d) `horizon_columns(obs, n_days)`: при `n_days < stats::G_MIN` границы `None`, точка — среднее без бутстрапа; шапка `bounds=lower/upper one-sided alpha each, none below g_min=7 days`. (e) `HEADER[16] = "touch_hours_utc"` (было `level_hours_utc`). (f) `lob::runs::log_trials(path, ts_utc, prefix, ids)` + `PROFILE_TRIAL_PREFIX`/`TOUCH_PROFILE_TRIAL_PREFIX` — единственный писатель строк испытаний профилей; `shortlist::log_profile_trials` делегирует, `runs::log_touch_profile_trials` снесён. Тесты `touch_profiles/tests.rs`: фикстура — касания по 500 мс (1 с — снаружи, 100 мс — внутри), `battle_args` с файлом окна, `seven_day_root` для интервала; `+battle_mode_without_preregistration_is_refused`. Всего 686.


## Из таска 43 — формат бинлога v3 (В-48, замер в `docs/findings/binlog-v3-2026-09-13.md`)

- `binlog::VERSION = 3`, `binlog::VERSION_V2 = 2`; заголовок (`MAGIC`+версия+`tick_e9`+`step_e9`+`max_records_per_frame`) у версий общий, различия — только в теле кадра. Один `Reader` читает **обе**: версию выбирает байт в заголовке (`version != 3 && version != 2` → `UnsupportedVersion`), тело разбирает `decode_frame_payload_v3`, для v2 — legacy-декодер. Новые аксессоры: `Reader::version()`, `Reader::legacy_dead_fields() -> LegacyDeadFields { nonzero_order_id, nonzero_ival, nonzero_fval }` (копятся на v2-кадрах; на v3 нули по построению — полей нет), `Reader::last_frame_bytes()` (полная длина кадра на диске: нужна замеру, чтобы базовая линия бралась из читателя, а не из позиции файла снаружи).
- Тело кадра v3: `epoch_exch_ns: i64 LE`, затем группы до конца среза; группа = `ev: uvarint`, `exch_dt: zigzag`, `local_dt: zigzag` (одна метка приёма на сообщение биржи), `attrs: uvarint` (бит 0 `ATTRS_BLOCK` = блочная сделка, остальные биты обязаны быть нулевыми → иначе `Corrupt`), `count: uvarint`, затем `count × (price_dt: zigzag, qty_dt: zigzag)` — дельты от предыдущей записи кадра, как в v2. `count` проверяется против остатка кадра **до** чтения записей (`count > остаток` → `Corrupt`, а не цикл по счётчику с диска).
- Граница групп — часть формата: `pub fn same_message(a, b)` (`ev`, `exch_ts_ns`, `local_ts_ns`, `block` совпали). По ней режет кодировщик (просмотр вперёд по срезу, `count` не хранится отдельным полем) и считает группы замер (`binlog-stats`), второй редакции границы в командах нет.
- `Record { ev, exch_ts_ns, local_ts_ns, price_ticks, qty_lots, block: bool }`, `#[derive(.., PartialEq, Eq)]` (`Eq` стал возможен: нецелого поля в записи больше нет). `order_id`/`fval` из `Record` и формата выброшены (0 % ненулевых на 29.2 млн записей), `ival: i64` заменён `block: bool` (семантика `lob/levels.rs`: блочные сделки пропускаются). `lob/export.rs::record_to_event` возвращает их на границе крейта: `order_id: 0`, `ival: i64::from(r.block)`, `fval: 0.0`. Читатели блочности — `feed::replay`, `commands::lob::replay`, `bybit::verify`.
- Кодек, публичное: `encode_frame_payload_v3(records, out) -> FieldBytes` (единственный писатель формата, его зовёт `Writer::write_frame`), `encode_frame_payload_v3_index_simulated(records, indexed, out) -> FieldBytes` (**только замер** M1i: дельта цены заменена одним байтом индекса; в формат не входит), `encode_frame_payload_v2(records, out) -> FieldBytes` (фикстуры совместимости и A/B-базовой линии; пишет нули в `order_id`/`fval` и `ival` из `block`), `decode_frame_payload_v3(payload) -> Result<Vec<Record>>`; legacy-декодер `decode_frame_payload_v2(payload, &mut LegacyDeadFields)` приватный.
- `pub struct FieldBytes { epoch, ev, exch_ts, local_ts, price, qty, group_overhead, dead_fields }` + `add` — полевой бюджет в **сырых** байтах; копит кодировщик, читает замер («за что платятся байты»). `LEN_PREFIX` стал `pub` (замер собирает кадры в памяти той же формулой), `MIN_RECORD_LEN = 8` (максимум по версиям: v2 — 8 полей, v3 — заголовок группы 5 + 2; меньшая граница сделала бы потолок ниже настоящего кадра v2), `MAX_RECORD_LEN = 80` (тоже максимум по версиям).
- `commands::lob::binlog_stats`: `BinlogStatsArgs { path, reencode }`; `BinlogStats { version, records, groups, frames, bytes_on_disk, by_ev, block_trades, local_ts_differs, legacy_dead, min/max_records_in_frame }`; замер — `ReencodeReport` + `measure_reencode` (числа для тестов) и `run_reencode`→`report_lines` (текст диспетчеру); считается **только по v2** (иначе `bail!`), файл читается в память целиком, чтобы время не мерило системные вызовы. Строки: `M1` (v2-диск против v3), `M1b` (кадр = сообщение), `M1i` (индекс), `M2` (экстраполяция ×10), `M3` (нс/запись кодирования+zstd и чтения), `M4` (round-trip и совпадение перекодировки v2 с диском), полевые бюджеты.
- Тесты: v3 round trip групп (флаги/метки/`block`/дельты), метки сообщения у всех записей группы, `attrs` с неизвестным битом → `Corrupt`, `count` больше кадра → `Corrupt`, кадр короче эпохи → `Corrupt`, версия 1 → `UnsupportedVersion`, версия 2 читается как legacy со счётчиками мёртвых полей, версия 3 их не набирает, `--reencode` на v2-фикстуре (`M4`: 0 расхождений, перекодировка совпала с диском) и отказ на v3-файле. Правка вызовов — механическая (`ival` → `block`, литералы `Record` без `order_id`/`fval`); гейты аллокаций (`write_path_allocates_nothing_*`) зелёные, горячий путь не менялся. Всего 707 тестов.

## Из таска 44 — остаток формата и ворота сервера (В-49)

- `binlog` += замерные варианты, **в формат не входящие**: `EvMode { Inline, Table(&[u64]) }` (в `encode_frame_payload`; в формате только `Inline`), `const EV_TABLE_MAX: usize = 32`, `pub fn encode_frame_payload_v3_ev_table(records, out) -> FieldBytes` (таблица `ev` на кадр: `ev_count` + сами значения, в группе — код, для значения сверх потолка — escape `ev_count` плюс полный `ev`, поэтому поток не теряет флаги молча) и `pub fn decode_frame_payload_v3_ev_table(payload) -> Result<Vec<Record>>` (без обратного чтения экономия проверялась бы на слово). Обычный `encode_frame_payload_v3` зовёт `encode_frame_payload(records, PriceMode::Delta, EvMode::Inline, out)`.
- `commands::lob::binlog_stats`: `BinlogStatsArgs` += `rewrite_out: Option<PathBuf>`; `ReencodeReport` += `v3_ev_table_bytes`, `v3_level3_bytes`, `v3_level6_bytes`, `v3_compress_ns` (только сжатие уровня 1 — база сравнения для M1z, без неё `v3_encode_ns` включает и кодирование), `v3_level6_encode_ns`, `ev_table_mismatches`; строки отчёта `M1e` и `M1z`. Кандидаты уровней — `const ZSTD_LEVEL_CANDIDATE_3/6` со ссылкой на T25. Компрессоры уровней 3 и 6 живут рядом с базовым; у M1b **свой** буфер `msg_raw` (общий `v3_raw` затирался телом последнего сообщения, и M1z мерил огрызок — 0.11 Б/запись; тест-инвариант `уровень 3 обязан сжимать то же тело, что уровень 1`).
