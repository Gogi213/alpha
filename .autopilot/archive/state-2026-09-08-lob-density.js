window.STATE =
{
  ...
  "updatedAt": "2026-09-10T04:22:23+04:00",
  "finishedAt": null,
  "stages": [
    {
      "id": "preflight",
    ...
    },
    {
      ...
      "note": "ревизия 17б: открытых вопросов нет. Пилот два часа по пулу, размер по Decision 22а. Блокер один — lob pick не запускался, инструмент не выбран"
    },
    {
      "id": "review",
      "status": "pending"
  ...
  "tickets": [
    ...
    },
    {
      ...
      "concerns": [
        "В-25: размер по Decision 22а — наименьшее допустимое биржей количество, не минимальный лот. У 8 из 10 инструментов пула минимальный лот дешевле обязательных $5 (IOSTUSDT $0.002, до чека 2539 шагов)",
        "minNotionalValue = 5 у всех 863 linear-символов; инструментная часть ограничения — в minOrderQty и qtyStep, то есть в единицах товара",
        "колонки: excluded_reason, покрытие топ-50 в bps, пригодность корзин (Decision 26а), номинал размера"
      ],
      "note": "таблица закоммичена (окно 300с, бан на длинные); финалисты SOL+XRP; размеры 22а",
      "finishedAt": "2026-09-10T04:22:23+04:00",
      "commit": "8a3fa99"
    },
    {
      "id": "0.5",
    ...
    },
    {
      ...
      "finishedAt": "2026-09-10T04:03:41+04:00",
      "tests": {
        "passed": 443,
        "failed": 0
      },
      "commit": "7ef2b03",
      "concerns": [
        "В-26: нарушение теста 3 — сделка по цене, которую книга ни разу не держала; сделка за границей топ-50 это out_of_range без порога (В-22)"
      ]
    },
    {
      "id": "0.7",
    ...
    },
    {
      ...
      "commit": "abd01ad",
      "concerns": [
        "ревизия 17 сменила бизнес-задачу (BUSINESS-TASK.md, редакция 2): проверить, что расстояние до середины считается в bps, а не в тиках: тик по пулу гуляет в 50 раз"
      ]
    },
    {
      "id": "1.2",
    ...
    {
      "id": "3.1",
      "title": "Пилот 2 часа по пулу из десяти, гейт G0 — до недели записи",
      "requirements": [
        "R03"
      ],
      "blockedBy": [
      ...
      "concerns": [
        "В-11 ЗАКРЫТ ревизией 17б: пилот два часа, как в плане. Ему добавлены две обязанности — назначить глубину креста профилей и перемерить GC на десяти инструментах с лимитом топиков; пять минут не дают ни того, ни другого",
        "гейт G0 переписан под пул: измеримость по медианному инструменту, грубая окупаемость на объединённой выборке — «лучший из десяти» это отбор, а отбор оплачивается Decision 27"
      ]
    },
    {
      "id": "4.1",
    ...
    },
    {
      ...
      "commit": "2cc9b32",
      "concerns": [
        "ревизия 17 сменила бизнес-задачу (BUSINESS-TASK.md, редакция 2): вместо двух ячеек — таблица профилей по сетке Decision 26, расстояние в bps, добавляются колонки fill и net_fill",
        "SETTLED В-10: реализация есть (коммит и тесты на строке), измерение не проведено — done-condition называет число, а не код",
    ...
    },
    {
      ...
      "commit": "39fb6a1",
      "concerns": [
        "ревизия 17 сменила бизнес-задачу (BUSINESS-TASK.md, редакция 2): гейт G3 переписан с m после издержек на net_fill",
        "SETTLED В-10: реализация есть (коммит и тесты на строке), измерение не проведено — done-condition называет число, а не код",
    ...
    {
      "id": "5.4",
      "title": "Шорт-лист на разведке, заморозка коммитом, подтверждение на поздней выборке",
      "requirements": [
        "R02",
        "R05"
      ],
      ...
      "handoffs": 0,
      "concerns": [
        "Decision 27: без него «профиль, ведущий к прибыли» нефальсифицируемо — 178 испытаний на десяти инструментах дают красивый результат на шуме гарантированно",
        "заморозка обязана быть механической: lob markout --confirmatory на профиле вне шорт-листа падает ненулевым кодом, а git log показывает коммит шорт-листа перед первым чтением подтверждающей"
      ],
      "startedAt": "2026-09-10T01:05:41+04:00",
      "finishedAt": "2026-09-10T01:16:43+04:00",
      "tests": {
        "passed": 442,
        "failed": 0
      },
    ...
    },
    {
      ...
      "commit": "bd9775d",
      "concerns": [
        "ревизия 17 сменила бизнес-задачу (BUSINESS-TASK.md, редакция 2): каждый из 178 профилей — отдельная строка runs.csv, а не только шорт-лист: число строк это вход DSR",
        "SETTLED В-10: реализация есть (коммит и тесты на строке), измерение не проведено — done-condition называет число, а не код",
  ...
  "additions": [
    ...
    "ревизия 17: Decision 25 (пул), 26 (профиль и сетка в bps), 27 (контракт перебора), шаг 5.4, гейты G0 и G3 переписаны",
    "ревизия 17б: Decision 22а (размер от чека), 29 (фикстура), 30 (nightly для фаззинга); тест 3 уточнён; В-11 закрыт"
  ],
  "coverage": null,
  "concerns": [
    "все открытые вопросы закрыты ревизией 17б: размер ордера, тест 3, длительность пилота, таймаут, фикстура, nightly",
    "бизнес-задача сменилась: BUSINESS-TASK.md редакция 2 — пул из десяти, отчёт по профилям, перебор объявлен и посчитан (Decision 25-27)",
