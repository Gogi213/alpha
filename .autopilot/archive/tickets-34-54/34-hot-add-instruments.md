...
- [x] тест `feed/live/tests.rs`: `LiveFeed::add` с фейковым коннектором — события нового символа
      приходят с новым индексом, старые не сдвинулись, `stop` гасит и новый поток
- [x] тест `session/tests.rs`: между двумя тиками в `instruments.csv` дописана строка → появился
      ...
      только stderr
- [x] тест: индекс события нового символа совпадает с позицией `SymbolState` (иначе кадр уйдёт
      в чужой файл)
- [x] `cargo test --release --target-dir target-ci` (649 + новые), clippy `-D warnings`, fmt;
...
- [x] живьём **не проверять на идущем коллекторе** (`data/always-on/20260912T122440Z` — старый
...
- [x] `CLAUDE.md` «Команды» + `docs/COMMANDS.md`: как докинуть монету (дописать строку в
      `<root>/instruments.csv`; удаление — не поддерживается)
- [x] **GC**: сборка одна, в `target-ci`, никаких параллельных cargo
