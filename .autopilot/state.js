window.STATE =
{
  ...
  "tickets": [
    ...
    { "id": "15", "title": "Горячий путь: on_event под Bot<MD>, ордер до триггера, G-LAT, dry-run", "requirements": ["R09","R74i","R78","R79","R80","A04"], "blockedBy": ["01","04"], "wave": 3, "zone": ["src/lob/strategy.rs","src/bybit/trade_ws.rs","src/commands/lob/react.rs"], "status": "done", "startedAt": "2026-09-11T09:15:00+04:00", "finishedAt": "2026-09-11T12:55:00+04:00", "tests": "530 passed, 0 failed, 5 ignored (126dd4d+15 изолированно)", "commit": "1b8b682 + ремонт 92ae94a", "review": { "R-A": "нет; R74i/R78/R79 partial — живые числа G-LAT требуют ключей (долг владельца)", "R-B": "1 blocking: аллокация в сборке ордера на событие → буферы + rebuild только при смене цены; остаток sign.rs → concerns", "R-C": "та же находка → снята; секреты не текут" }, "retries": 0, "repairs": 1, "handoffs": 0 },
  ...
  "gates": [
    ...
    { "id": "G-LAT",     "status": "pending", "note": "замер 5 мин SOLUSDT (92ae94a): разбор p99 68.7 мкс, книга 222.5 мкс, весь путь по 63 срабатываниям median 111.8 мкс / p99 1.77 мс (< 5 мс), но triggers=63 < 1000 — НЕ ОБЪЯВЛЕН; нужен прогон ~1.5 ч (владелец). RTT хоста — lob probe ставит реальные ордера, не запускался" },
  ...
  "debt": [
    ...
    { "what": "k для пола H3 — владелец делегировал методологию (В-30/D05): k выбирает пилот по сетке {2,5,10,20,50} и гейтам G0/G1", "default": "реализация — таск 18; до него отладка с k = 1.0", "owner": "таск 18", "ticket": "18" },
