window.STATE =
{
  ...
  "stages": [
    ...
    },
    {
      ...
      "note": "сдан 2026-09-12; волны 8–9 после сдачи: T20–T23. Волна 10 (владелец 2026-09-12): T24–T26, коллектор запущен 2026-09-12 12:24Z. Волна 11 по аудиту против задачи сдана (T27–T30, 647 тестов); T32 дашборд — код и живой localhost, ревью не проведено (лимит); T31 pending. Коллектор перезапущен на боевой десятке 2026-09-12 20:17Z (data/always-on/20260912T201737Z, T34 hot-add, В-41 stop-файл); старая восьмёрка 12:24–20:12Z — отладочная запись"
    }
  ],
  "requirements": {
    "total": 90,
  ...
  "tickets": [
    ...
    {
      "id": "43",
      "title": "Формат бинлога v3: группы сообщений, время на сообщение, мёртвые поля вон (В-48)",
      "requirements": ["R79", "R80", "R84"],
      "blockedBy": [],
      "wave": 13,
      "zone": ["src/binlog/mod.rs", "src/commands/lob/binlog_stats.rs", "src/lob/export.rs", "src/feed/replay.rs", "src/commands/lob/session/sink.rs", "src/commands/record.rs"],
      "status": "done",
      "retries": 0, "repairs": 0, "handoffs": 0,
      "tests": "707 passed, 0 failed, 5 ignored; clippy и fmt чистые",
      "note": "владелец 2026-09-13: «оптимизировать коллектор сильно: формат файлов, формат записи, тип записи, скорость, объём». Пререгистрация метрик M1/M1b/M1i/M2/M3/M4/M5/M6 и порогов — в тикете 43 до кода. Замер `lob binlog-stats --reencode` на десяти живых файлах (29 203 623 записи, 34 785 кадров): байт/запись 74.4 % (LSK) … 98.8 % (NEAR) от v2, пул 82.1 % (−17.9 %), кодирование 0.44–0.67×, чтение 0.26–0.40×, round-trip 0 расхождений, перекодировка v2 совпала с диском на всех кадрах. Порог M1 ≤ 0.75 взят только на LSK ⇒ раскатка v3 на живой коллектор — решение владельца (местная запись остановлена 2026-09-13T16:22Z, переезд на сервер). Отклонены замером: кадр-на-сообщение (129–273 % от v2), «уровень индексом» (101–111 % от v3, плюс требует книги у читателя). Следующий кандидат — таблица «код → ev» на кадр (ev = 5.00 Б/запись сырых байт при 4–6 значениях), своей предрегистрацией. Разбор: docs/findings/binlog-v3-2026-09-13.md"
    },
    {
      "id": "44",
      "title": "Остаток формата: `ev` таблицей и уровни zstd — оба отклонены замером; ворота сервера (В-49)",
      "requirements": ["R79", "R80", "R84"],
      "blockedBy": ["43"],
      "wave": 13,
      "zone": ["src/binlog/mod.rs", "src/commands/lob/binlog_stats.rs", "docs/COMMANDS.md"],
      "status": "done",
      "retries": 0, "repairs": 0, "handoffs": 0,
      "tests": "709 passed, 0 failed, 5 ignored; clippy и fmt чистые",
      "note": "пререгистрация M1e/M1z/M4e/M5c/M5s — до кода. M1e «ev таблицей на кадр»: −2.1…−4.7 % (медиана 3.9 %), round-trip 0 расхождений → ниже порога 5 %, в формат не внедряем (кодек остаётся замерным). M1z: уровень 6 даёт 0.9–2.8 %, уровень 3 хуже уровня 1 на девяти из десяти (99.6–100.8 %), CPU не мешал (+0.03–0.06 % ядра) → уровень 1 остаётся; старый бенч T25 мерил уровни на теле v2 и на v3 не воспроизводится. Формат заморожен. M5c: SOL v2→v3 (--rewrite-out, 809 095 записей, те же 1352 кадра), lob verify — те же числа, lob levels — 8629 уровней и CSV побайтово тот же (SHA-256 совпал). M5s: живой 5-мин смоук новым бинарником (data/smoke-v3, debug по D-ОТЛАДКА) — формат v3, CPU 2.1 %, parse p99 118.8 мкс, resyncs 0, verify ok, levels 221. Ранбук сервера — docs/COMMANDS.md. Попутно найден и исправлен баг самого замера (буфер тела кадра затирался в M1b, M1z мерил огрызок) — на него тест-инвариант"
    },
    {
      "id": "45",
      "title": "Раскатка пула на топ-50: размер пула параметром `--top`, живой тест (В-50)",
      "requirements": ["R79", "R80", "R84"],
      "blockedBy": [],
      "wave": 14,
      "zone": ["src/commands/lob/pick/", "docs/COMMANDS.md"],
      "status": "done",
      "retries": 0, "repairs": 0, "handoffs": 0,
