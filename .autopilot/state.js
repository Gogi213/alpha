window.STATE =
{
  ...
  "updatedAt": "2026-09-11T22:10:00+04:00",
  "finishedAt": null,
  ...
  "stages": [
    ...
    { "id": "build",     "status": "active", "startedAt": "2026-09-11T02:20:00+04:00", "note": "T16 1fc8fc3, T09б 2af1996 — G-DEBUG пройден. Ремонт react (домен часов, серия full только по срабатываниям) в полёте; затем T14. 15 из 16 готово" },
  ...
  "tickets": [
    ...
    { "id": "09", "title": "Пилот: отладка ≤5 мин, затем два часа; G-DEBUG, G0, G-POWER-B", "requirements": ["R40","R51","R68","R78","R81","A01"], "blockedBy": ["01","02","08"], "wave": 3, "zone": ["src/commands/lob/pilot.rs"], "status": "done", "startedAt": "2026-09-11T11:30:00+04:00", "finishedAt": "2026-09-11T22:10:00+04:00", "tests": "568 passed, 0 failed, 5 ignored (1fc8fc3+09б)", "commit": "7ef26dd (а), 2af1996 (б)", "review": { "R-A": "нет; шесть боевых критериев partial — боевой пилот после k/ключей", "R-B": "нет; свой ридер clock.csv → concerns", "R-C": "clean; G-DEBUG сверен с диском" }, "commit": "7ef26dd (часть а)", "tests": "542 passed, 0 failed, 5 ignored (1b8b682+09а изолированно)", "review": { "R-A": "нет; R78/R81 done, R40/A01 partial → (б)", "R-B": "нет; реплей ×3–4, 1087 строк → concerns", "R-C": "1 blocking: G0 по сырому m без проскальзывания → net ≥ 0; снято" }, "retries": 0, "repairs": 1, "handoffs": 0 },
    { "id": "15", "title": "Горячий путь: on_event под Bot<MD>, ордер до триггера, G-LAT, dry-run", "requirements": ["R09","R74i","R78","R79","R80","A04"], "blockedBy": ["01","04"], "wave": 3, "zone": ["src/lob/strategy.rs","src/bybit/trade_ws.rs","src/commands/lob/react.rs"], "status": "done", "startedAt": "2026-09-11T09:15:00+04:00", "finishedAt": "2026-09-11T12:55:00+04:00", "tests": "530 passed, 0 failed, 5 ignored (126dd4d+15 изолированно)", "commit": "1b8b682", "review": { "R-A": "нет; R74i/R78/R79 partial — живые числа G-LAT требуют ключей (долг владельца)", "R-B": "1 blocking: аллокация в сборке ордера на событие → буферы + rebuild только при смене цены; остаток sign.rs → concerns", "R-C": "та же находка → снята; секреты не текут" }, "retries": 0, "repairs": 1, "handoffs": 0 },
  ...
  "gates": [
    ...
    { "id": "G-DEBUG",   "status": "passed", "note": "ПРОЙДЕН: живой прогон ≤5 мин, 8 инструментов, session→verify→levels→markout→profiles→backtest 8/8 без дефекта — T09б 2af1996, data/pilot-debug/20260911T084259Z. Фаза отладки закрыта; часовые окна и двухчасовой пилот разрешены (нужен k)" },
    ...
    { "id": "G-LAT",     "status": "pending", "note": "живой lob react 5 мин SOLUSDT (data/react-debug/20260911T110318Z): разбор p99=139.9 мкс (<200), триггер p99=1.1 мкс, send p99=0.2 мкс; стадия «книга» отрицательная на всех 117 строках — смешение доменов часов (parsed_ts_ns SystemClock vs MonotonicClock) — дефект, ремонт; 117 срабатываний < 1000 за 5 мин — не объявлен. lob probe (RTT) ставит реальные post-only ордера create+cancel — не запускался, решение владельца" },
  ...
  "concerns": [
    ...
    { "ticket": "15", "file": "src/bybit/sign.rs Credentials::sign", "what": "реальный подписант аллоцирует внутри sign_into (делегирует sign()); ноль аллокаций доказан для фейкового подписанта; буферный HMAC в sign.rs — таск 14 (R-B, R-C)", "kind": "structural" },
    { "ticket": "15", "file": "src/lob/strategy.rs best_prices", "what": "6-строчная копия приватного хелпера backtest.rs (зона T11) — обоснована в докстроке; свести при таске 11 (R-B)", "kind": "structural" },
    { "ticket": "15", "file": "src/commands/lob/react.rs", "what": "триггер react — смена лучшего тика, не density-сигнал levels; осознанное упрощение замера, документировано (R-A, R-B)", "kind": "craft" },
    { "ticket": "09", "file": "src/commands/lob/pilot.rs run_pilot_debug", "what": "нет регресс-теста «--debug не пишет runs.csv» — свойство держится конструкцией (log_pilot_run только в battle); добавить в 09(б) (R-A)", "kind": "craft" },
    { "ticket": "09", "file": "src/commands/lob/pilot.rs process_instrument", "what": "реплей инструмента 3–4 раза (floor/percentile/markout/агрегация), потому что run_markout не возвращает сырые наблюдения — не горячий путь; свести в 09(б) (R-B)", "kind": "structural" },
    { "ticket": "09", "file": "src/commands/lob/pilot.rs", "what": "1087 строк — потолок на файл не назначен; структурно (R-B)", "kind": "structural" },
    { "ticket": "10", "file": "src/lob/shortlist.rs ProfileRow/write_profiles_csv", "what": "старая схема колонок не совпадает с таском 10 — рискует остаться мёртвым кодом; снести в таске 14 (R-A)", "kind": "structural" },
    { "ticket": "11", "file": "src/commands/lob/backtest.rs --debug", "what": "метка debug по флагу вызывающего, не по длине окна (как levels); читать session.json.duration_s — на будущее; отдельного теста на шапку/debug нет (R-C)", "kind": "craft" },
    { "ticket": "11", "file": "src/lob/backtest.rs 1309 строк, src/commands/lob/backtest.rs 579", "what": "крупные файлы, потолок не назначен (R-B)", "kind": "structural" },
    { "ticket": "11", "file": "src/commands/lob/backtest.rs --order-qty-e9", "what": "не читается из instruments.csv/order_size_22a автоматически — обязательный флаг; проводка — таск 13/09б (R-C)", "kind": "craft" },
    ...
    { "ticket": "16", "file": "src/commands/lob/shortlist.rs шапка pbo/cpcv", "what": "PBO/CPCV печатают None — нужна матрица «испытания × периоды» (покруговые net по общим календарным периодам на все id сетки), которую profiles/backtest не строят; отдельный таск 17 после боевых данных (R-A)", "kind": "plan-cut" },
    { "ticket": "16", "file": "src/commands/lob/{profiles,shortlist}.rs", "what": "нет сквозного теста profiles→shortlist через BacktestFillModel до Confirmed — только fill на синтетике + decide_profile отдельно (R-A)", "kind": "craft" },
    { "ticket": "09", "file": "src/commands/lob/pilot.rs process_instrument", "what": "до 5 реплеев на инструмент — run_verify/run_levels/run_markout возвращают только сводки; свести — таск 14 или отдельный (09б)", "kind": "structural" },
    { "ticket": "16", "file": "src/commands/lob/{profiles,shortlist}.rs Args", "what": "три флага RTT/лота продублированы в двух Args — второй случай паттерна H3Args, свести в общую структуру #[command(flatten)] (R-B)", "kind": "structural" },
    { "ticket": "16", "file": "src/commands/lob/shortlist.rs jackknife", "what": "leave-one-day-out гоняет весь ScratchRoot-конвейер на каждые сутки — стоимость не оценена (R-B)", "kind": "structural" },
  ...
  "debt": [
    { "what": "живой гейт G-LAT (lob react, dry-run без ордеров) требует BYBIT_API_KEY/BYBIT_API_SECRET в окружении — тёплый аутентифицированный WS trade", "default": "не запускать; G-LAT не объявлен", "owner": "владелец", "ticket": "09" },
  ...
  "blind": null,
  "tests": { "passed": 568, "failed": 0, "ignored": 5, "at": "2026-09-11T22:10:00+04:00" }
}
