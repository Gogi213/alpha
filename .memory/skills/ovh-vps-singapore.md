...
замка на машине — см. «Грабли»). **С 2026-09-15 это боевой сервер**: коллектор, данные и сборка
живут здесь (см. `projects/active/alpha.md`); прежний `13.140.29.171` закрыт решением В-54.

**Текущее (2026-09-16):** пишем **100 монет** двумя потоками (`.50` + `deep/`), замеренный расход
**5.9 ГБ/сутки**, свободно 42 ГБ ⇒ **≈7 суток**; **ротации нет** — это первое, что упрётся.
Числа ниже в датированных разделах — на момент тех замеров, не текущее состояние.
...
- Есть: `curl`, `python3`, `rsync`, `git`, `systemctl`. **С 2026-09-16 есть и toolchain:**
  `build-essential` + rustup 1.93.1 в `/home/ubuntu/.cargo`, сборка в `~/alpha-build` (на системном
  диске `sda`, диск данных `sdb` не занимаем) — см. «Сборка и раскатка».
...
- `rustup` **поставлен на проде 2026-09-16** (`/home/ubuntu/.cargo`, toolchain 1.93.1): сборка идёт
  здесь, на системном диске; последняя раскатка боевого бинарника — 2026-09-15T22:47:56Z (RPI-правка, В-53).
