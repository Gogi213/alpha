...
cargo test --release --target-dir target-ci 2>&1 | tail -5   # 837 passed, 0 failed, 8 ignored (2026-09-18, В-62; на Linux +1 — тест SIGTERM под cfg(unix))
...
плотностью; тейк `b × σ_H`, не ниже `k × 7.5` bps комиссий; `σ_H` — реализованная волатильность середины за окно
...
волатильность середины за окно, равное дедлайну; `lob::sigma`), полы «тик за плотностью» и `k × 7.5` bps комиссий;
...
история; дампы `b5/0917-*` на счётной машине вердиктом не читаются. Советы практиков (Михеев, Rust, 18.09) —
