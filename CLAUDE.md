...
единственная автоматика — ночная сверка (A2 ниже). **В проде — бинарник 2026-09-15 22:47Z**: правки A3 (K1/V4/V5/V6/V12) в коде есть,
на сервер не выкачены — деплой решает владелец. 2026-09-17 01:51Z одно переподключение → 100 строк
...
`verify` `ok` по всем пяти; живой коллектор не задет. Деплой коллектора — после A8 (dev-plan), словом владельца. Профиль `perf`: ядро/сисколлы
