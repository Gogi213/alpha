...
`sequence_gap` «шов покрытия» в `gaps.csv` (`reconnects=1`); по политике V4 сверка за 09-17 пометит
все 100 монет `fail` — решение о «шве» ждёт владельца. Профиль `perf`: ядро/сисколлы
