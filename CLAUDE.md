...
Боевой прогон сетки (сервер, 100 монет, маркеры `verify`) — **не гонялся** (оценка: ≈ 25 мин на сутки
пула при двух потоках, ≈ 45 при одном); замечание к интервалу `net_fill` (бутстрэп без центрирования)
— аудиту, решением владельца.
