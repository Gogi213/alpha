...
Состояние: боевой пул `instruments.csv` — **топ-50, заморожен** боевым часовым отбором (В-50,
`lob pick --top 50 --window-secs 3600`, коммит `3da0b3f`): 50 строк, `window_secs = 3600`, без
метки `# debug`, `k = 1.0` (заглушка, В-30), `h3_lots` 2…3454, `depth_check` 28 `below_floor` /
22 `ok`; состав тот же, что у отладочного отбора часом раньше. Размер пула — параметр
`lob pick --top N`. Живой тест на 50: одно соединение, CPU 6.9 % ядра (бюджет 6.1 — 5 %, превышен
⇒ ждёт решения владельца), 1.97 ГБ/сутки, `verify` 47/50 (`fail`: LSK/BTW/MU, воспроизводится и на
v2 ⇒ не формат); разбор — `docs/findings/pool-top50-2026-09-13.md`.

Коллектор **работает** на **старом сервере** `13.140.29.171` (пути и грабли — `docs/COMMANDS.md`,
«Развёртывание на старом сервере»): Rust собран на месте (1.93.1), бинарник `/opt/alpha/alpha-collector`,
**пул топ-30** в `/opt/alpha/root` (первые тридцать строк боевого топ-50; копия —
`instruments-top30-server.csv`, MD5 `1672790c…` совпадает с серверными `root/instruments.csv` и
`/opt/alpha/instruments-top30.csv`), юнит `alpha-collector.service` **включён и активен**
(`enable --now` с 2026-09-14T18:20:59Z, `Restart=on-failure` — штатная остановка файлом `stop`, В-41).
Замер 2026-09-14T22:30Z (4 ч 10 мин, разбор — `docs/findings/deploy-2026-09-14.md`): 72.9 млн записей,
`gaps=reconnects=resyncs=frames_failed=0`, CPU avg 5.26 % ядра (max 6.63), RSS 22 МБ, parse p99
122 мкс, формат **v3**, 341 МБ за 4 ч ⇒ **2.05 ГБ/сутки** при 40 ГБ свободных ≈ 20 дней.
**Ротации/выгрузки нет** (на сервере нет `crontab`, `rsync`, `rclone`) — упрётся первой; `verify`
серверных суток ещё не гонялся (в `root` нет `verify-*.status`).
