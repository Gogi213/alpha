...
Коллектор развёрнут на **старом сервере** `13.140.29.171` (пути и грабли — `docs/COMMANDS.md`, «Развёртывание на старом сервере»): Rust собран на месте (1.93.1), бинарник `/opt/alpha/alpha-collector`, пул топ-20 в `/opt/alpha/root`, юнит `alpha-collector.service` создан, но **не включён** — ждёт решения владельца; 10-минутный тест зелёный (CPU 4.6 % ядра, RSS 17 МБ, parse p99 139 мкс, v3, gaps 0).
