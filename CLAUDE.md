...
**ожидаемое решение В-62** (владелец 18.09: «переходить с тиков на проценты и держать в уме стандартизированную
волатильность»): стоп/тейк как множители σ середины, полы «тик за плотностью» и `k × издержки`; геометрия в тиках
(В-58) — в архив. Не реализовано, ждёт подтверждения. История силы (минимум за 1/5/15/60 с, `repeat_count`) —
