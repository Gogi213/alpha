...
impl Value {
    // ЛОКАЛЬНАЯ ПРАВКА alpha: у нас крейт собирается без чтения npy из файлов, и этот
    // метод даёт `dead_code`-предупреждение в чистом выводе `cargo clippy -D warnings`
    // (в апстриме он используется примерами, которые в локальную копию не входят).
    #[allow(dead_code)]
