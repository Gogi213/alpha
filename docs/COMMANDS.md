...
9. **`verify` закрытых суток — таймером (A2, 2026-09-17).** `alpha-verify.timer`
   ...
   Без этого копирования сутки с реальными ресинками проходили бы `ok`; в строке вердикта печатается
   `gaps_csv=<n>`.
