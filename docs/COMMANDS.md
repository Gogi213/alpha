...
| `lob binlog-stats --path <файл> [--reencode]` | **только чтение** (T43, В-48): версия формата из заголовка, записи, кадры, **группы-сообщения** (сколько записей на сообщение биржи), `ev` по типам, блочные сделки, `local_ts ≠ exch_ts`; для файла версии 2 — счётчики мёртвых полей `order_id`/`ival`/`fval` (ими доказано «0 % ненулевых» и на них стоит v3). `--reencode` — замер A/B формата на **тех же** записях: M1 байт/запись v2 против v3, M1b кадр-на-сообщение, M1i симуляция «уровень индексом», M2 экстраполяция ×10, M3 нс/запись (кодирование+zstd и чтение), M4 round-trip и совпадение перекодировки v2 с файлом на диске; считается только по файлу версии 2 (иначе отказ). Пороги решения и числа — тикет 43, `docs/findings/binlog-v3-2026-09-13.md` |
