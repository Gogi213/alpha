...
Проверено по коду: OS-специфичных мест два — `commands/lob/session/resources.rs`
(Windows: запрос через PowerShell; Linux: `/proc/self/stat` и `VmRSS`) и сторож свободного места
там же (A1, 2026-09-17: `statvfs` под `#[cfg(unix)]`, на не-Unix `disk_free_bytes` возвращает
`None`, и `session.json` честно пишет «замера нет», а тревоги не поднимает). Единственная
платформенная зависимость — `libc` в `[target.'cfg(unix)'.dependencies]`; Linux-сборка — та же
команда `cargo build --release`.
...
6. **Диск — единственное, что кончится (срок ≈ 22.09.2026):** боевой пул 100 монет пишет
   **≈ 6.5 ГБ/сутки** (аудит `collector-audit-2026-09-17.md` §8), свободного места 38 ГБ.
   **Сторож (A1, 2026-09-17):** на каждом тике сессии (10 с) `statvfs` по корню записи — в
   `session.json` ложатся `disk_free_bytes`, `disk_warn_bytes` (порог `--disk-warn-gib`,
   умолчание 15 ГиБ ≈ 2.3 суток) и `disk_free_low`; на переходе «место ниже порога» — одна
   строка `stderr` (журнал юнита), и она же видна дежурному по `session.json`. Замер на
   не-Unix — `None`: тревоги нет, поле честно пустое. **Почему сторож, а не «и так видно»:** при
   полном диске падает запись всех 100 монет сразу и `gaps.csv` перестаёт писаться тоже (V2/V6),
   то есть потеря становится невидимой.
   **Регламент закрытых суток (вручную, решение владельца 2026-09-17):** (1) после UTC-полуночи
   `lob verify --symbol <SYM> --root <root>` по каждой монете — маркеры `verify-<SYM>.status`
   ложатся в боевой корень; (2) закрытые сутки → `lob archive --path <root>/<SYM>-<day>.binlog
   --level 19` (контейнер `<SYM>-<day>.binlog.zst`, замер — 81.3 % от оригинала); (3) только
   после `verify == ok` и сверки round-trip — `--delete-original`.
   **Открытую часть `lob archive` не трогает:** если `session.json` в каталоге имеет
   ...
   этой сессии — `unlink` открытого файла потерял бы всё, что допишется после (A1, аудит V8).
