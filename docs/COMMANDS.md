...
| `lob touch-profiles --root --h3-mode floor\|percentile [--h3-lots] [--h3-k] [--warmup-ms] [--repeat-window-ms] [--allow-unverified] [--out] [--now-utc] [--runs-out] [--preregistration] [--window-end]` | `docs/findings/touch-profiles-<дата>.csv` (T37, В-44) — таблица профилей касаний: строка на каждый id сетки `lob::touch_axes::touch_profile_grid` — маргиналы девяти осей В-44 (сторона, размер на касании ×`H3`, возраст, фронтран в долях размера, круглость, номер касания, подход за 1 с, длительность, исход) плюс один крест исход × возраст, на пул целиком (`pool:…`) и на каждый инструмент (`<SYMBOL>:…`); id — `<область>:marginal:<ось>=<корзина>` / `<область>:cross:<исход>\|<возраст>`. Колонки: `profile_id, n, share_bounced, {m, m_lower, m_upper}×{100ms,1000ms,10000ms,60000ms}` (markout «в сторону отскока» от среза как есть на `start_ms`, В-43; интервал — `costs::net_fill_interval` по суткам-кластерам, как `m_lower` у `profiles`; верхняя — та же функция на обратном знаке), `n_days, level_hours_utc` (часы начала касаний). `net`/`net_fill` здесь нет — они у сделки-отскока в бэктесте (T38): markout отбирает, бэктест выносит вердикт. Шапка: `h3_mode` (+ `h3_k`), `warmup_ms`, `repeat_window_ms`, `alpha`, `replications`, `seed`, `debug`; окно — `format_window_line` при `--preregistration` (тот же файл, что у `profiles`), иначе `window: not preregistered <first>..<last> days= sessions=`; `rtt=` не печатается. `--root` — корень с `instruments.csv` и подкаталогами сессий **или** сам каталог сессии (`session.json` в нём); маркер `verify-<SYMBOL>.status == ok` обязателен (fail-closed), `--allow-unverified` → `debug`, `runs.csv` не пишется. Число испытаний `(26 + 6) × (инструментов + 1)` — по строке `touch_profile <id>` в `runs.csv` (`lob::runs::log_touch_profile_trials`), журнал, не перезапись. stdout `touch-profiles: rows= touches= trials= debug= out=` |
