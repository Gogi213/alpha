...
| `lob touches --root --symbol --h3-mode floor\|percentile [--h3-lots] [--h3-k] [--warmup-ms] [--repeat-window-ms] [--out]` | `touches-<SYMBOL>.csv` (T35, В-42) — касания живых уровней (`lob::levels::TouchRecord`): касание начинается, когда живой уровень (> `H3`) стал лучшей ценой своей стороны, кончается, когда перестал ею быть или умер (`ended_by_death`, `end_ms` = смерть). Колонки: `day_utc, side, price_tick, touch_index, start_ms, end_ms, duration_ms, level_birth_ms, age_ms, size_at_touch, size_max_before, traded_during, frontrun_lots` (лоты той же стороны лучше уровня на последнем кадре до касания), `round_zeros` (0/1/2/3+), `ended_by_death, stack_levels` (живых ≥ `H3` той же стороны), `dist_bps` (при рождении, как у `profiles`), `m_100ms, m_1s, m_10s, m_60s` (знак «в сторону отскока» — минус знака смерти), `approach_1s, approach_10s` (сдвиг середины до касания, плюс — к уровню); пустая ячейка — нет среза. Тот же реплей, что `levels`/`markout`; смерти не менялись; дашборд/профили/шортлист касаний не читают (после предрегистрации) |
