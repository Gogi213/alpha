...
if [ -z "$TOUCHES_ONLY" ]; then
  ...
  # tr -d '\r' — защита от CRLF (ревью 24.09: eol=lf в .gitattributes — основной барьер, это —
  # запасной, если файл всё же попадёт в дерево с \r).
  BASE_SETS=$(grep -v '^#' "$SELF_DIR/sets-nightly.txt" | tr -d '\r' | tr '\n' ' ')
