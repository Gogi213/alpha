...
COMMON="--root study/root-f10-1619 --signal approach --queue-model prob:3 $RTT"
...
SETS="a45-bid:age=2700,side=bid s100-bid:flow=100,side=bid a15-bid:age=900,side=bid"
for D in D20 D50; do
  for S in $SETS; do
    NAME="${S%%:*}"
