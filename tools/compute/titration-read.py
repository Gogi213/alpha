...
net на сделку, контроль, превышение, суток с превышением > 0, деньги на лоте $1000 (net × n / 10).
...
def read_epoch(study, tag):
    ...
    for name in sorted(os.listdir(study)):
        ...
        out[set_name] = {
            ...
            "days": int(r["days"]), "days_pos": int(r["days_excess_pos"]), "usd": net * n / 10.0,
            "verdict": verdict,
        }
    return out
